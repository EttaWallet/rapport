 ...
 * https://github.com/v0l/lnproxy-ts/blob/master/src/offchain/create_hodl_invoice.ts
 */
...
export default async function ({
  ...
  if (!request) {
    throw new ValidationError(
      'ExpectedBolt11PaymentRequestToCreateHodlInvoice',
    );
  }
  ...
  if (dateDiff(details.expires_at, new Date().toISOString()) < minDateDiffSec) {
  ...
  details.features.forEach((n) => {
    if (!knownFeatures.includes(n.bit) || !n.is_known) {
      throw new ValidationError(
        `UnExpectedFeatureBitInPaymentRequest ${n.type}`,
      );
    }
  });
  ...
  if (!skipProbe) {
    ...
    if (!probe.route) {
      ...
      // maybe open a channel here and repeat the probeForRoute after 5seconds.
    }
  ...
  subscribeToInvoices({
    ...
    id: details.id,
    partnerPubKey: details.destination,
  }).catch(Logger.error);
